      ...
      <div className="flex flex-col gap-1.5">
        <div className="flex items-center gap-2">
          ...
          <span className="ml-auto font-mono text-white/90">
            ${data.baseline.toLocaleString()}
          </span>
        </div>
        <div className="flex items-center gap-2">
          <span className="h-2 w-2 rounded-full bg-[var(--engine-grow)]" />
          <span className="text-white/50">Poseidon-Optimized:</span>
          <span className="ml-auto font-mono text-[var(--engine-grow)]">
            ${aiOptimized.toLocaleString()}
          </span>
        </div>
      </div>
    </div>
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <Label
              value={formatDollarK(finalAiOptimized)}
              position="right"
              offset={8}
              fill="#8B5CF6"
              fontSize={12}
          ...
          >
            <Label
              value={formatDollarK(finalBaseline)}
              position="right"
              offset={8}
              fill="#64748B"
              fontSize={11}
...
export function GrowHero({
  ...
  const hasConfidenceBand = simulationData.some(
    (d) => d.low != null && d.high != null,
  );
      ...
      <div className="flex flex-col lg:flex-row w-full">
        ...
        <div className="flex flex-col gap-6 lg:gap-10 flex-1 p-6 lg:p-10 justify-center">
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs lg:text-sm text-white/40 font-mono">
            ...
            <span className="text-white/20">&middot;</span>
            <span>
              <span className="text-white/70">{recommendationCount}</span>{" "}
              recommendations
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-6 lg:min-w-[320px] lg:w-[350px] lg:border-l lg:border-white/5 p-6 lg:p-10 justify-center bg-white/[0.02]">
          ...
          {spotlightRec && (
            <div
              className="bg-white/[0.04] border-l-2 border-[var(--engine-grow)] rounded-lg p-3 animate-fadeUp"
            ...
            >
              ...
              <p
                ...
                style={{ color: "var(--engine-grow)" }}
              >
                ${spotlightRec.monthlySavings.toLocaleString()}/mo ·{" "}
          ...
          {goalsWithProgress && goalsWithProgress.length > 0 && (
            <div
              className="hidden md:flex flex-col gap-3 animate-fadeUp"
